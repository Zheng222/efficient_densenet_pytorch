...
class _EfficientDensenetBottleneck(nn.Module):
    ...
    """
    def __init__(self, shared_allocation_1, num_input_channels, num_output_channels):
    ...
    def forward(self, inputs):
        if isinstance(inputs, Variable):
            inputs = [inputs]
        fn = _EfficientDensenetBottleneckFn(self.shared_allocation_1,
...
class _DenseLayer(nn.Sequential):
    def __init__(self, shared_allocation_1, num_input_features, growth_rate, bn_size, drop_rate):
        super(_DenseLayer, self).__init__()
        self.shared_allocation_1 = shared_allocation_1
        self.drop_rate = drop_rate

        self.add_module('bn', _EfficientDensenetBottleneck(shared_allocation_1,
...
class _DenseBlock(nn.Container):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, storage_size=1024):
        ...
        for i in range(num_layers):
            layer = _DenseLayer(self.shared_allocation_1, num_input_features + i * growth_rate,
...
class _EfficientDensenetBottleneckFn(Function):
    ...
    """
    def __init__(self, shared_allocation_1,
    ...
    def forward(self, bn_weight, bn_bias, conv_weight, *inputs):
        ...
        conv_input = Variable(relu_output.data, requires_grad=any(self.needs_input_grad))
        conv_weight_var = Variable(conv_weight, requires_grad=self.needs_input_grad[2])
        conv_output = F.conv2d(conv_input, conv_weight_var, bias=None, stride=self.stride,
    ...
    def backward(self, grad_output):
        ...
        if any(self.needs_input_grad[:2]) or any(self.needs_input_grad[3:]):
            ...
            # ReLU/BN backward
            relu_output.backward(gradient=self.conv_input.grad)
